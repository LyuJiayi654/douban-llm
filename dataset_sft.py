...
class SFTDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        sample = self.df.iloc[index]
        prompt = self.tokenizer.encode(sample['post'], add_special_tokens=False)
        answer = self.tokenizer.encode(sample['comment'], add_special_tokens=False)
        # prompt = self.tokenizer.encode(self.df['post'], add_special_tokens=False)
        # answer = self.tokenizer.encode(self.df['comment'], add_special_tokens=False)
...
if __name__=="__main__":
    ...
    train_ds = SFTDataset(df[:200],tokenizer, max_length=256)
