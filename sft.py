...
def train_epoch(epoch):
    ...
    for step, (X, Y, loss_mask) in enumerate(train_loader):
        ...
        with ctx:
            ...
            # print(loss, loss_mask.sum().item())
            # if pd.isna(torch.sum(loss * loss_mask).item()):
            if pd.isna(torch.sum(X).item() + torch.sum(Y).item()):
                print("Y", Y.view(-1))
                print("X", X)
                print("logits", logits)
                print("qwq", step)
            ...
            #         pickle.dump(logits.view(-1, logits.size(-1)), f)
            #     with open("y.pkl","wb") as f:
            #         pickle.dump(Y.view(-1), f)
            #     exit(0)
...
if __name__ == "__main__":
    ...
    batch_size = 24  # if gradient_accumulation_steps > 1, this is the micro-batch size #32
    # model
    max_seq_len = 512 # 512
    dim = 512
    n_layers = 8
    ...
    # adamw optimizer
    learning_rate = 2e-5  # max learning rate  2e-5
    weight_decay = 1e-4
    beta1 = 0.9
    beta2 = 0.95
    grad_clip = 1  # clip gradients at this value, or disable if == 0.0  1.0
    ...
    # -----init dataloader------
    df = pd.read_csv('zctytlxz_id_c_c_c.csv')
    # input=[]
    # target=[]
    ...
    train_ds = SFTDataset(df[0:40003], tokenizer, max_length=512)
    ...
    # init model
    model = init_model()
    # model.load_state_dict(torch.load('./out/sft/epoch_30.pth'))
    model.to(device)
